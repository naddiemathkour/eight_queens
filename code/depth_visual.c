...
void print_board(long long *board){
  for(int i = 0; i < BOARDSIZE; i++){
    if (!(i%8)) printf("\t"); 
    printf("%s%d ", YEL, (*board & (shift << i)) ? 1 : 0);
    if (i%8 == 7) printf("\n");
  }
  printf("\n");
}

void print_board_visual(long long *board, int place){
  for(int i = 0; i < BOARDSIZE; i++){
    if (!(i%8)) printf("\t"); 
    (i == place) ?
    printf("%s%d ", RED, (*board & (shift << i)) ? 1 : 0)
    :
    printf("%s%d ", WHT, (*board & (shift << i)) ? 1 : 0);
...
int generate_permutation(long long *board, int queens, int place){
  if (queens == 8){
    ...
    print_board(&solutions[solution++]);
    sleep(3);
    return 1;
  }
  ...
  *temp ^= (shift << place); //place queen
  ...
  print_board_visual(board, place);
  //sleep(1); //steps
  usleep(500000);
  //usleep(85000);
